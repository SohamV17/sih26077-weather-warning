...
# ============================================================
# XGBOOST RISK COMPONENT
# ============================================================

def nowcast_risk_component(
    rain_probability,
    predicted_rain_mm
):
    """
    Convert XGBoost predictions into a risk component.

    This does NOT train the model.
    It only interprets the model output.
    """
...
def calculate_nowcast_risk(
    ...
    if max_probability >= 0.80:
        ...
        reasons.append(
            f"XGBoost predicts a high probability "
            f"of rainfall ({max_probability * 100:.0f}%)"
        )

    elif max_probability >= 0.50:

        reasons.append(
            f"XGBoost predicts a moderate probability "
            f"of rainfall ({max_probability * 100:.0f}%)"
        )
    ...
    if max_rain >= 20:
        ...
        reasons.append(
            f"Potential significant rainfall "
            f"({max_rain:.1f} mm)"
        )
    ...
    if current_wind >= 35:
        ...
        reasons.append(
            f"Strong current winds "
            f"({current_wind:.1f} km/h)"
        )
    ...
    return {
        ...
        "components": {
            ...
            "current_conditions": round(
                current_component,
                1
            ),
            "official_alert": round(
                official_alert_component,
                1
            )
        }
